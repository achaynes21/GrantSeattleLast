...
namespace InventoryERP.Service.Services.Services.Implementations
{
    public class NewsPortalService:BaseService,INewsPortalService
    {
       ...
       public void Save(News entity)
        ...
        public IList<News> GetRecentNews()
        {
            return NewsRepository.GetQuery().Where(x => !x.IsDelete && x.Status == Propertys.PropertyStatusText.Active)
                .OrderByDescending(x => x.CreatedAt).ToList().Skip(1).Take(3).ToList();
        }

        public IList<News> GetSidebarNews()
        {
            return NewsRepository.GetQuery().Where(x => !x.IsDelete && x.Status == Propertys.PropertyStatusText.Active)
                .OrderByDescending(x => x.CreatedAt).ToList().Skip(4).Take(2).ToList();
        }

        public IList<News> GetNewsListFront(int page, int PAGE_SIZE)
        {
            return NewsRepository.GetQuery().Where(x => !x.IsDelete && x.Status == Propertys.PropertyStatusText.Active)
                .OrderByDescending(x => x.CreatedAt).ToList().Skip((page - 1)*PAGE_SIZE).Take(PAGE_SIZE).ToList();
        }

        public double RowCount()
        {
            return NewsRepository.GetQuery().Where(x => !x.IsDelete && x.Status == Propertys.PropertyStatusText.Active)
                .OrderByDescending(x => x.CreatedAt).ToList().Count;
        }
